...
import { View, Text, TextInput, TouchableOpacity, FlatList, KeyboardAvoidingView, Platform, StyleSheet, Keyboard, ActivityIndicator, Image, Dimensions, Modal, TouchableWithoutFeedback, ScrollView, SafeAreaView, Animated } from 'react-native';
...
function extractFilters(text) {
  ...
  const query = text.replace(/under (\d+)/i, '').replace(/cedis|ghc|gh\u20b5/gi, '').trim();
...
function isGiftRequest(text) {
  return /gift|present|for (my|a|an|the) (friend|mother|father|child|boy|girl|wife|husband|birthday|anniversary|occasion)/i.test(text);
}

function isAccessoryRequest(text) {
  return /accessor(y|ies)|compatible|add-on|case|charger|cover|screen protector/i.test(text);
}
...
function isGreeting(text) {
  return /\b(hi|hello|hey|good morning|good afternoon|good evening|greetings)\b/i.test(text);
}
...
function renderFormattedText(text) {
  ...
  return paragraphs.map((para, pIdx) => (
    <View key={pIdx} style={{ marginTop: pIdx === 0 ? 0 : 18, marginBottom: 2 }}>
      {para.map((line, idx) => {
        if (line.trim().startsWith('##')) {
          ...
          return (
            <Text key={idx} style={{ marginBottom: 2 }}>{renderBoldText(line)}</Text>
          );
        }
      })}
    </View>
...
function renderBoldText(text) {
  ...
  return boldParts.map((part, i) => {
    if (part.startsWith('**') && part.endsWith('**')) {
      // Bold
      return <Text key={i} style={{ fontWeight: 'bold' }}>{renderItalics(part.slice(2, -2))}</Text>;
    } else {
      // May contain italics
...
const countryToCurrency = {
  GH: 'GHS', NG: 'NGN', US: 'USD', GB: 'GBP', FR: 'EUR', DE: 'EUR', IT: 'EUR', ES: 'EUR', NL: 'EUR', JP: 'JPY', CN: 'CNY', IN: 'INR', CA: 'CAD', AU: 'AUD', KE: 'KES', ZA: 'ZAR', // ... add more as needed
};
const currencySymbols = {
  USD: '$', GHS: 'GH₵', NGN: '₦', GBP: '£', EUR: '€', JPY: '¥', CNY: '¥', INR: '₹', CAD: 'C$', AUD: 'A$', KES: 'Ksh', ZAR: 'R', // ... add more as needed
};
...
export default function Cartlyst() {
  const [messages, setMessages] = useState([
    { role: 'assistant', text: 'Hi there👋, Cartlyst here, ready to speed things up🔥. How can I help you today?' }
  ]);
  ...
  const fetchSessions = async (userId) => {
    const res = await fetch(`${API_BASE_URL}/assistant/sessions?userId=${userId}`);
    if (res.ok) {
  ...
  const switchSession = async (session) => {
    ...
    const res = await fetch(`${API_BASE_URL}/assistant/session/${session.id}/history`);
    if (res.ok) {
      const history = await res.json();
      setMessages(history.map(msg => {
        let extra = {};
        try { extra = msg.extraData ? JSON.parse(msg.extraData) : {}; } catch { extra = {}; }
        return {
          role: msg.role,
          text: msg.text,
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (storedSessionId) {
        ...
        const res = await fetch(`${API_BASE_URL}/assistant/session/${storedSessionId}/history`);
        if (res.ok) {
          const history = await res.json();
    ...
    (async () => {
      ...
      if (status === 'granted') {
        ...
        if (geocode && geocode[0] && geocode[0].isoCountryCode) {
          const currency = countryToCurrency[geocode[0].isoCountryCode] || 'USD';
          setUserCurrency(currency);
        }
      }
      // Fetch exchange rates
      try {
        const res = await fetch(`${API_BASE_URL.replace('/assistant','')}/api/exchange/rates`);
        if (res.ok) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/assistant/session/${sessionId}/message`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (userMsgSaved) {
      ...
      const assistantPayload = { userId, role: 'assistant', text: response.text, extraData: JSON.stringify({ products: response.products, categories: response.categories }) };
  ...
  async function handleAssistant(text) {
    ...
    } catch (e) { /* fallback to current state */ }
  ...
  const handleAction = (action) => {
    if (action.type === 'view') {
      ...
      router.push({ pathname: '/(tabs)/(1-home)/product/' + action.product.id });
    }
    if (action.type === 'addToCart') {
      addToCart(userId, action.product, 1);
      setMessages((msgs) => [...msgs, { role: 'assistant', text: `Added ${action.product.title} to your cart.` }]);
    }
    if (action.type === 'addToWishlist') {
      addToWishlist(userId, action.product);
      setMessages((msgs) => [...msgs, { role: 'assistant', text: `Added ${action.product.title} to your wishlist.` }]);
    }
  };
    ...
    <View style={[styles.messageRow, item.role === 'assistant' ? styles.assistantRow : styles.userRow]}>
      ...
      <View style={[styles.bubble, item.role === 'assistant' ? styles.assistantBubble : styles.userBubble]}>
        ...
        {item.products && item.products.length > 0 && (
          <FlatList
            ...
            renderItem={({ item: product }) => (
              <View style={{ width: PRODUCT_CARD_WIDTH }}>
                <ProductCard
                  ...
                  image={product.image}
                  title={product.title}
                  price={(!product.currency || product.currency === 'USD')
                    ? convertPrice(Number(product.price), 'USD', userCurrency)
                    : Number(product.price)}
                  discount={product.discount}
                  rating={product.rating}
                  onPress={() => router.push({ pathname: '/(tabs)/(1-home)/product/' + product.id })}
                  onAddToCart={() => addToCart(userId, product, 1)}
                  onPressHeart={() => addToWishlist(userId, product)}
                  isFavorite={!!(user && user.wishlist && user.wishlist.some(w => w.id === product.id))}
                />
              </View>
            )}
            horizontal
            ...
            getItemLayout={(_, index) => ({ length: PRODUCT_CARD_WIDTH + CARD_GAP, offset: (PRODUCT_CARD_WIDTH + CARD_GAP) * index, index })}
          ...
          <FlatList
            ...
            renderItem={({ item: category }) => (
              <View style={{ width: CATEGORY_CARD_WIDTH }}>
                <CategoryCards
                  categories={[category]}
                  onPress={(cat) => router.push({ pathname: '/(tabs)/(2-categories)/products/' + cat.id })}
                />
              </View>
            )}
            horizontal
            showsHorizontalScrollIndicator={false}
            style={{ marginTop: 8, marginBottom: 8 }}
            getItemLayout={(_, index) => ({ length: CATEGORY_CARD_WIDTH + CARD_GAP, offset: (CATEGORY_CARD_WIDTH + CARD_GAP) * index, index })}
            ItemSeparatorComponent={() => <View style={{ width: CARD_GAP }} />}
            contentContainerStyle={{ paddingHorizontal: 0 }}
          />
        )}
      ...
      {item.role === 'user' && (
        user && user.profileImageUrl ? (
          ...
          <View style={[styles.avatarCircleUser, { backgroundColor: COLORS.primary }] }>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.background }}>
      {/* Hamburger menu */}
      <TouchableOpacity
        style={{ position: 'absolute', top: 20, left: 20, zIndex: 200, backgroundColor: '#fff', borderRadius: 20, padding: 10, elevation: 4 }}
      ...
      {sidebarOpen && (
        <TouchableOpacity
          style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.3)', zIndex: 99 }}
          activeOpacity={1}
      ...
      >
        <Text style={{ fontWeight: 'bold', fontSize: 20, color: COLORS.primary, marginBottom: 24 }}>Chat Sessions</Text>
        <TouchableOpacity
          style={{ backgroundColor: COLORS.primary, borderRadius: 8, padding: 12, marginBottom: 16 }}
          onPress={async () => {
            setSidebarOpen(false);
            await startNewSession();
          }}
        >
          <Text style={{ color: '#fff', fontWeight: 'bold', textAlign: 'center' }}>New Session</Text>
        </TouchableOpacity>
        <Text style={{ fontWeight: 'bold', fontSize: 16, marginBottom: 8 }}>Session History</Text>
        <ScrollView style={{ flex: 1 }}>
          {sessions
            ...
            .map((session) => (
              <TouchableOpacity
                key={session.id}
                style={{ paddingVertical: 10, borderBottomWidth: 1, borderColor: COLORS.gray[200] }}
                onPress={async () => {
                  setSidebarOpen(false);
              ...
              >
                ...
                <Text style={{ color: COLORS.text.secondary, fontSize: 12 }}>Started: {new Date(session.createdAt).toLocaleString()}</Text>
                <Text style={{ color: COLORS.text.secondary, fontSize: 12 }}>Last Active: {new Date(session.lastActiveAt).toLocaleString()}</Text>
              </TouchableOpacity>
            ))}
        </ScrollView>
      </Animated.View>
      ...
      >
        ...
        <View style={[styles.inputRow, isInputFocused && { borderColor: COLORS.primary }]}>
          <TextInput
            style={styles.input}
            value={input}
          ...
          <TouchableOpacity style={styles.sendBtn} onPress={sendMessage} disabled={loading || !input.trim()}>
            <FontAwesome6 name="arrow-up" size={20} color={COLORS.text.inverse} />
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
    </SafeAreaView>
