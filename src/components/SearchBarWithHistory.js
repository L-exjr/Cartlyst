...
export default function SearchBarWithHistory({
  ...
  useEffect(() => {
    ...
    const matches = suggestionsSource
      .filter(
        (p) =>
          (p.title && typeof p.title === 'string' && p.title.toLowerCase().includes(lower)) ||
          (p.category && typeof p.category === 'string' && p.category.toLowerCase().includes(lower))
      )
      .map((p) => ({
        title: p.title,
        category: p.category
      }))
      .filter((v, i, arr) => arr.findIndex(x => x.title === v.title && x.category === v.category) === i)
      .slice(0, 8);
  ...
  const addToSearchHistory = async (term, category) => {
    ...
    let newHistory = [entry, ...searchHistory.filter((t) => t.term !== term || t.category !== category)];
  ...
  const showCategoryOnly = placeholder && placeholder.toLowerCase().includes('categories');
    ...
    <View style={styles.container}>
      ...
      {showSuggestions && (search.length > 0 ? suggestions.length > 0 : searchHistory.length > 0) && (
        <ScrollView style={styles.suggestionDropdown} keyboardShouldPersistTaps="handled">
          {search.length > 0 && suggestions.map((s, i) => (
            <TouchableOpacity key={s.title + s.category + i} style={styles.suggestionItem} onPress={() => handleSuggestionTap(s.title, s.category)}>
              {showCategoryOnly ? (
                ...
                <Text>
                  ...
                  {s.category && (
                    <Text> in <Text style={styles.categoryText}>{s.category}</Text></Text>
                  )}
                </Text>
              )}
            </TouchableOpacity>
          ))}
          {search.length === 0 && searchHistory.map((h, i) => (
            <TouchableOpacity key={h.term + h.category + i} style={styles.suggestionItem} onPress={() => handleSuggestionTap(h.term, h.category)}>
              {showCategoryOnly ? (
                ...
                <Text>
                  ...
                  {h.category && (
                    <Text> in <Text style={styles.categoryText}>{h.category}</Text></Text>
                  )}
                </Text>
              )}
            </TouchableOpacity>
